...
from db.crud import get_topic_scores, get_all_turns, get_session_token_usage
...
async def generate_report(session_id: str, orchestrator_state, db) -> dict:
    ...
    # Get token counts: from orchestrator state if available, otherwise from audit log
    if orchestrator_state:
        nova_lite_input_tokens = getattr(orchestrator_state, "nova_lite_input_tokens", 0)
        nova_lite_output_tokens = getattr(orchestrator_state, "nova_lite_output_tokens", 0)
        nova_pro_input_tokens = getattr(orchestrator_state, "nova_pro_input_tokens", 0)
        nova_pro_output_tokens = getattr(orchestrator_state, "nova_pro_output_tokens", 0)
    else:
        # Session not in memory (page refresh, session expired, etc.) - get tokens from audit log
        audit_tokens = await get_session_token_usage(db, session_id)
        nova_lite_input_tokens = audit_tokens.get("nova_lite_input_tokens", 0)
        nova_lite_output_tokens = audit_tokens.get("nova_lite_output_tokens", 0)
        nova_pro_input_tokens = audit_tokens.get("nova_pro_input_tokens", 0)
        nova_pro_output_tokens = audit_tokens.get("nova_pro_output_tokens", 0)
    ...
    cost = compute_cost_usd(
        nova_lite_input_tokens,
        nova_lite_output_tokens,
    ...
    nova_pro_input_tokens = nova_pro_input_tokens + meta.get("input_tokens", 0)
    nova_pro_output_tokens = nova_pro_output_tokens + meta.get("output_tokens", 0)
